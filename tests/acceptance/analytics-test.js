...
module('Acceptance | Selectors for GTM Analytics', function(hooks) {
  ...
  test('test for analytics selectors', async function(assert) {
    ...
    let title = findWithAssert('.item__head__headline > a')[0];
    let show = findWithAssert('.brick__item .item__head__brand')[0];

    // assert event values
    let firstStory = server.db.buckets[0].bucketItems[0].attributes;

    assert.equal(overlayLink.getAttribute('data-action'), 'Homepage', 'overlay link data-action');
    assert.equal(overlayLink.getAttribute('data-label'), `${firstStory.title} | ${firstStory.headers.brand.title}`,'overlay link data-label');

    assert.equal(title.getAttribute('data-action'), 'Homepage', 'story title data-action');
    assert.equal(title.getAttribute('data-label'), `${firstStory.title} | ${firstStory.headers.brand.title}`, 'story title data-label');

    assert.equal(show.getAttribute('data-action'), 'Homepage', 'show link data-action');
    assert.equal(show.getAttribute('data-label'), `${firstStory.headers.brand.title}`, 'show link data-label');
